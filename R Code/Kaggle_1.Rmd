...
require(ggplot2)
require(reshape2)
qplot(x=Var1,y=Var2,data=melt(cor(credit_train[,3:(ncol(credit_train)-1)],use="p")),fill=value,geom="tile")+scale_fill_gradient2(limits=c(1,-1)) 
...
#Creating a scaled  version of pred_data
scaled_pred_data<-scale(pred_data[,-c(1:2)])
scaled_pred_data<-cbind(pred_data[,c(1:2)],scaled_pred_data)
...
gbm.mod<-gbm(as.character(SeriousDlqin2yrs)~age+scale(MonthlyIncome)+RevolvingUtilizationOfUnsecuredLines+NumberOfOpenCreditLinesAndLoans+NumberOfDependents+NumberOfTime60.89DaysPastDueNotWorse,distribution = "bernoulli",data=credit_train,n.trees = 1000,interaction.depth = 2,shrinkage = .01)
...
#SVM
svm.mod<-svm(SeriousDlqin2yrs~age+DebtRatio+RevolvingUtilizationOfUnsecuredLines+NumberOfOpenCreditLinesAndLoans+NumberOfDependents+NumberOfTime60.89DaysPastDueNotWorse,data=credit_train,type="C-classification",kernel="linear",cost=10,scale=F)
pred_svm<-predict(svm.mod,newdata = pred_data)
...
#Ensemble
final_pred<-rep(0,nrow(predicted))
for(i in 1:nrow(predicted)){
  
  #print(sum(predicted[i,]))
  final_pred[i]<-sum(predicted[i,])
  
}
final_pred<-ifelse(final_pred>=1,1,0)
predicted<-cbind.data.frame(predicted,final_pred)
mean(pred_data$SeriousDlqin2yrs!=predicted$final_pred)
pred_roc<-roc(pred_data$SeriousDlqin2yrs,predicted$final_pred)
